...
public:
    ...
    ESPSoftwareSerial(int8_t txPin = -1, bool invert = false);
    ESPSoftwareSerial(const ESPSoftwareSerial&) = delete;
    ESPSoftwareSerial& operator= (const ESPSoftwareSerial&) = delete;
    virtual ~ESPSoftwareSerial();
    ...
    void begin(uint32_t baud, SoftwareSerialConfig config, int8_t txPin) {
        begin(baud, config, txPin, m_invert);
    }
    void begin(uint32_t baud, SoftwareSerialConfig config = SWSERIAL_8N1) {
        begin(baud, config, m_txPin, m_invert);
    }

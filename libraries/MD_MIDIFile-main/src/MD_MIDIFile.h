...
#include <Arduino.h>
#include <../../generic_storage/FAT/FatFS/src/ff.h> // Replace SdFat to Fatfs 
...
//Jusitn for debug usage
#include "../../../../murobox_ws/MuroBox-USB/MuroBoxUtility.hpp" //For debug print

#if DUMP_DATA
#define DUMPS(s) printer.myPrintf(s) ///< Print a string
#define DUMP(s, v)       \
  {                      \
    printer.myPrintf(s); \
    printer.myPrintf(v);     \
...
public:
   ...
   */
  const TCHAR *getFilename(void) { return (_fileName); }; // char to TCHAR
   ...
   * \return No return data.
   */
  void setFilename(const TCHAR *aname) { _fileName = aname; } // char to TCHAR
   ...
   */
  int load(const TCHAR *fname); // char to TCHAR
...
protected:
  ...
  const TCHAR *_fileName;      ///< MIDI file name buffer in user code   // char to TCHAR

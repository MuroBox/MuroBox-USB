...
// track_event = <time:v> + [<midi_event> | <meta_event> | <sysex_event>]
{
  ...
  FRESULT rc;
  rc = f_lseek(&mf->_fd, _startOffset + _currOffset);
  if (rc)
  {
    printer.myPrintf("\r\n getNextEvent f_lseek error");
    return (false);
  }
  ...
  _currOffset = f_tell(&mf->_fd) - _startOffset;
...
// ---------------------------- MIDI
  ...
  case 0xe0 ... 0xef:
    ...
    _mev.data[1] = readonebyte(&mf->_fd);
    _mev.data[2] = readonebyte(&mf->_fd);
  ...
  case 0x00 ... 0x7f: // MIDI run on message
  {
    ...
    {
      _mev.data[i] = readonebyte(&mf->_fd);
    } 
...
// ---------------------------- SYSEX
  ...
  case 0xf7:  // sysex_event = 0xF7 + <len:1> + <data_bytes> + 0xF7 
  {
    ...
    for (uint16_t i=index; i<minLen; ++i)
      sev.data[i] = readonebyte(&mf->_fd);
    if (sev.size > minLen)
    {
      FRESULT rc;
      rc = f_lseek(&mf->_fd, f_tell(&mf->_fd) + sev.size - minLen);
      if (rc)
        printer.myPrintf("\r\n parseEvent f_lseek error1");
...
// ---------------------------- META
  case 0xff:  // meta_event = 0xFF + <meta_type:1> + <length:v> + <event_data_bytes>
  {
    ...
    switch (eType)
    {
      ...
      {
        uint8_t n = readonebyte(&mf->_fd);
        uint8_t d = readonebyte(&mf->_fd);

        mf->setTimeSignature(n, 1 << d);  // denominator is 2^n

        FRESULT rc;
        rc = f_lseek(&mf->_fd, f_tell(&mf->_fd) + mLen - 2);
        if (rc)
          printer.myPrintf("\r\n parseEvent f_lseek error2");
      ...
      {
        DUMPS("KEY SIGNATURE");
        int8_t sf = readonebyte(&mf->_fd);
        uint8_t mi = readonebyte(&mf->_fd);
      ...
      default:
      {
        ...
        for (uint8_t i = 0; i < minLen; ++i)
          mev.data[i] = readonebyte(&mf->_fd);   // read next
        
        mev.chars[minLen] = '\0'; // in case it is a string
        if (mLen > ARRAY_SIZE(mev.data))
        {
          FRESULT rc;
          rc = f_lseek(&mf->_fd, f_tell(&mf->_fd) + mLen - ARRAY_SIZE(mev.data));
          if (rc)
            printer.myPrintf("\r\n parseEvent f_lseek error3");
        }
  //    DUMPS("IGNORED");
      }
      break;
...
// return -1 if success, 0 if malformed header, 1 if next track past end of file
{
  ...
  {
    // char to TCHAR
    TCHAR    h[MTRK_HDR_SIZE+1]; // Header characters + nul
    TCHAR *rc = f_gets(h, MTHD_HDR_SIZE + 1, &mf->_fd);
    if(!rc)
      printer.myPrintf("\r\n f_gets error ");

    h[MTRK_HDR_SIZE] = '\0';
#if _LFN_UNICODE
    if (wcscmp(h, MTRK_HDR) != 0)
#else
  ...
  _startOffset = f_tell(&mf->_fd);
  _currOffset = 0;

  // Advance the file pointer to the start of the next track;

  if (f_lseek(&mf->_fd, _startOffset + _length))
  {
    //1 if next track past end of file
    printer.myPrintf("\r\n next track past end of file");
    return (1);
  }
  return(-1);
}

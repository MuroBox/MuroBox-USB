...
// char to TCHAR
int MD_MIDIFile::load(const TCHAR *fname) 
...
// Return one of the E_* error codes
{
  ...
  FRESULT rc;
  rc = f_open(&_fd, _fileName, FA_READ);
  if (rc)
    {
      printer.myPrintf("Error f_open at load\n");
      return (E_NO_OPEN);
    }
    
    {
      // char to TCHAR
      TCHAR h[MTHD_HDR_SIZE + 1]; // Header characters + nul
      TCHAR* err = f_gets(h, MTHD_HDR_SIZE + 1, &_fd);
      if (!err)
      {
        printer.myPrintf("Error f_gets at load\n");
        return (E_NO_OPEN);
      }

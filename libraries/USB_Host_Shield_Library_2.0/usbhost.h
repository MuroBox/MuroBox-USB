...
// CHEN: Duplicated init value of SS pin?
//typedef SPi< P14, P13, P12, P15 > spi;
typedef SPi< P14, P13, P12, P2 > spi;
#elif defined(ESP32)
...
public:
        ...
        uint8_t GpxHandler();
        uint8_t IntHandler();
		uint8_t GPINHandler(uint8_t);
		uint8_t ID_NPolSelection(uint8_t gpin);
        uint8_t Task();
        ////////////////////////////////
        //Added for periphiral mode
        int8_t Init_device();
        uint8_t MAX_Int_Pending(void);
        ////////////////////////////////
};
...
/* CHEN: 20201018 */
template< typename SPI_SS, typename INTR >
void MAX3421e< SPI_SS, INTR >::spi_init() {
        // USB_HOST_SERIAL.print("SPI_SS: ");
        // USB_HOST_SERIAL.println(SPI_SS, HEX);
        XMEM_ACQUIRE_SPI();
        // Moved here.
        ...
        SPI_SS::Set();
        spi::init();
		/* The interrupt is monitored by polling for now  */
        INTR::SetDirRead();
        XMEM_RELEASE_SPI();
}

/*
USB Enumeration Code (and More) for the MAX3420E p.12:
The MAX_Int_Pending() function polls the MAX3420E INT pin and returns a 1 value if the pin is
found to be low.*/
template <typename SPI_SS, typename INTR>
uint8_t MAX3421e<SPI_SS, INTR>::MAX_Int_Pending(void)
{
        //Is there an IRQ pending
        uint8_t pinvalue;
        pinvalue = INTR::IsSet(); //Read Pin;
        if (pinvalue == 0)        //Pull low when there is an IRQ. See MAX3421E Programming Guide p.42.
        {
                return 1; 
        }
        else
...
// Reference: USB Enumeration Code (and More) for the MAX3420E p.7
template <typename SPI_SS, typename INTR>
int8_t MAX3421e<SPI_SS, INTR>::Init_device()
{
        uint8_t val;
        //spi_init(); //Only need once. This will cause PACK_INT 0-1 trigger MuroBox into UART mode. 
        regWr(rPINCTL, (bmFDUPSPI + bmINTLEVEL + GPX_SOF)); // MAX3420: SPI=full-duplex, INT=neg level, GPX=SOF

        if (reset() == 0)
        { //OSCOKIRQ hasn't asserted in time
                return (-1);
        }
        regWr(rGPINPOL, bmGPINPOL6);
        ...
        //ENABLE_IRQS
        regWr(rEPIEN, (bmSUDAVIE + bmIN0BAVIE + bmOUT1DAVIE));
        regWr(rUSBIEN, (bmURESIE + bmURESDNIE + bmNOVBUSIE + bmVBUSIE));
...
template< typename SPI_SS, typename INTR >
int8_t MAX3421e< SPI_SS, INTR >::Init_host() {
        /* CHEN: 20201018 */
        //spi_init(); //Only need once. This will cause PACK_INT 0-1 trigger MuroBox into UART mode.
        // regWr(rGPINIEN, ID_N_PIN | INT_N_OUT3_PIN);
        ...
        regWr(rUSBIEN, (bmNOVBUSIE + bmVBUSIE));
...
/* initialize MAX3421E. Set Host mode, pullups, and stuff. Returns 0 if success, -1 if not */
template< typename SPI_SS, typename INTR >
int8_t MAX3421e< SPI_SS, INTR >::Init_host(int mseconds) {
        /* CHEN: 20201018 */
        // spi_init(); //Only need once. This will cause PACK_INT 0-1 trigger MuroBox into UART mode.
...
uint8_t MAX3421e< SPI_SS, INTR >::Task(void) {
		...
		/* CHEN: Because we don't have GPX conneced, we have to handle GPIN interrupt in
				 IntHandler(). */
...
uint8_t MAX3421e< SPI_SS, INTR >::IntHandler() {
		...
		// /* Handle GPIN interrupt */
		uint8_t GPINIRQ;
		GPINIRQ = regRd(rGPINIRQ); //determine interrupt source
		if(GPINIRQ) {
		// 	/* For test */
		// 	USB_HOST_SERIAL.print("GPINIRQ state: ");
                //         USB_HOST_SERIAL.println( GPINIRQ, HEX );

                //         //GPINHandler(GPINIRQ); // Not used for plugin detection
                         regWr(rGPINIRQ, GPINIRQ); //clear IRQ
                        }
...
template <typename SPI_SS, typename INTR>
uint8_t MAX3421e<SPI_SS, INTR>::GPINHandler(uint8_t GPINIRQ)
{
        uint8_t tmpbyte = gpioRd();

        USB_HOST_SERIAL.print("GPIN Handler:");
        USB_HOST_SERIAL.println(tmpbyte, HEX);

        /* Clear GPIN IRQ interrupt */
        regWr(rGPINIRQ, GPINIRQ);
        return (GPINIRQ);
}

...
typedef int32_t (*mbox_callback) (int32_t);
...
#endif
        //CHEN: 20200924 add Muro Box hook function
        mbox_callback mbox_midi_insert_cb;
        mbox_callback mbox_midi_remove_cb;
        uint8_t RecvData(uint16_t *bytes_rcvd, uint8_t *dataptr); // should not be called directly. Move to protected.
...
public:
        ...
        //CHEN: 20200924 add Muro Box hook function
        int32_t MBOX_reg_midi_insert_hook(mbox_callback cb);
        int32_t MBOX_reg_midi_remove_hook(mbox_callback cb);
};
#endif //_USBH_MIDI_H_

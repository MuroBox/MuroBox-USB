...
uint8_t USBH_MIDI::Init(uint8_t parent, uint8_t port, bool lowspeed)
{
        ...
        bPollEnable = true;
        USBTRACE("Init done.\r\n");
        // CHEN: a MIDI device is plugged in
        if (mbox_midi_insert_cb) {
            mbox_midi_insert_cb(0);
        }
        return 0;
FailGetDevDescr:
FailSetDevTblEntry:
...
uint8_t USBH_MIDI::Release()
{
        ...
        // CHEN: MIDI device is removed
        if (mbox_midi_remove_cb) {
            mbox_midi_remove_cb(0);
        }
...
//CHEN: 20200924 add Muro Box hook function
int32_t USBH_MIDI::MBOX_reg_midi_insert_hook(mbox_callback cb) {
    mbox_midi_insert_cb = cb;
    return 0;
}

int32_t USBH_MIDI::MBOX_reg_midi_remove_hook(mbox_callback cb) {
    mbox_midi_remove_cb = cb;
    return 0;
}

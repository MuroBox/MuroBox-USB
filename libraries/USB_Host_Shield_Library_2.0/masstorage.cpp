...
uint8_t BulkOnly::ConfigureDevice(uint8_t parent, uint8_t port, bool lowspeed) {
        ...
        if(!p) {
                USBTRACE("USB_ERROR_ADDRESS_NOT_FOUND_IN_POOL: from bulkonly\r\n");
...

        {
                ...
                if(usberr == hrSUCCESS) {
                        if(IsValidCSW(&csw, pcbw)) {
                                ...
                                //ResetRecovery(); //take out resetrecovery refer to https://github.com/felis/USB_Host_Shield_2.0/issues/410
                                //return MASS_ERR_SUCCESS;

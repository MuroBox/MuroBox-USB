...
#define INIT_BUF(dobj)                  { \
                                                lfn = ff_memalloc((_MAX_LFN + 1) * sizeof(WCHAR)); \
...
#endif
                // take out pstr, if not will crashed on ESP8266
...
FRESULT f_mkfs(FBYTE vol, FBYTE sfd, UINT au) {
        ...
        FBYTE fmt, md, sys, *tbl, pdrv, part;
        ...
        memcpy_P(tbl, ("\xEB\xFE\x90" "MSDOS5.0"), 11); /* Boot jump code, OEM name */
        i = SS(fs); /* Sector size */
        ...
        if(fmt == FS_FAT32) {
                ...
                memcpy_P(tbl + BS_VolLab32, ("NO NAME    " "FAT32   "), 19); /* Volume label, FAT signature */
        } else {
                ...
                memcpy_P(tbl + BS_VolLab, ("NO NAME    " "FAT     "), 19); /* Volume label, FAT signature */
        }
